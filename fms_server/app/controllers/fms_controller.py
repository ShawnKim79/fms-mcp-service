...
async def find_ride_routes(
    ...
    fms_service: FmsService = Depends()
):
...
async def update_ride_route(route_id: UUID, route: Route, fms_service: FmsService = Depends()):
...
async def delete_ride_route(route_id: UUID, fms_service: FmsService = Depends()):
...
async def create_ride_request(ride_request: RideRequest, fms_service: FmsService = Depends()):
...
async def get_ride_request(request_id: UUID, fms_service: FmsService = Depends()):
...
async def approve_ride_request(request_id: UUID, fms_service: FmsService = Depends()):
